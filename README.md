...
local function setupSpeedIndicator(char)
	...
	-- discordLabel removed per user request
end
...
-- ⚡ PODER EXACTO: 23 - 32 - 70 - 90 (CRAZY)
local NIVELES = {
    Low     = { poder = 23 },
    Mid     = { poder = 32 },
    High    = { poder = 70 },
...
-- rename UI to Fury Duels
local function applyFuryDuelsTheme()
    -- Rename main title
    local mainTitle = exeMainFrame and exeMainFrame:FindFirstChildWhichIsA("TextLabel")
    if mainTitle then mainTitle.Text = "FURY DUELS" end
    
    -- Change tab button names
    for _, btn in pairs(tabButtons) do
        if btn and btn.Text then
            if btn.Text:lower() == ".exe" then
                btn.Text = "FURY"
            end
        end
    end
    
    -- Also update mobile button panel name
    local mobileGui = game:GetService("CoreGui"):FindFirstChild("ExeMobileButtons")
    if mobileGui then
        local titleLabel = mobileGui:FindFirstChildWhichIsA("TextLabel")
        if titleLabel then titleLabel.Text = "FURY DUELS" end
    end
end
...
local function buildGui()
	...
	ttl.BackgroundTransparency=1;ttl.Text="FURY DUELS";ttl.ZIndex=4
	...
	miniBtn.Text="FURY DUELS";miniBtn.TextColor3=RED;miniBtn.Font=Enum.Font.GothamBold;miniBtn.TextSize=11
...
-- SLOW RED COLOR CYCLING ON BLACK BACKGROUND
task.spawn(function()
    local hue = 0
    while task.wait(0.05) do
        hue = (hue + 0.002) % 1
        local redColor = Color3.fromHSV(hue, 1, 0.8)
        -- Update main UI color theme slowly
        if exeMainFrame then
            local menuBg = exeMainFrame:FindFirstChild("ExeMenuBackground")
            if menuBg then
                menuBg.ImageColor3 = redColor
            end
            -- Update progress bar color
            if progressFill then
                progressFill.BackgroundColor3 = redColor
            end
            -- Update title color
            local hdr = exeMainFrame:FindFirstChildOfClass("Frame")
            if hdr then
                local ttl = hdr:FindFirstChildWhichIsA("TextLabel")
                if ttl and ttl.Text == "FURY DUELS" then
                    ttl.TextColor3 = redColor
                end
            end
        end
        -- Update mobile button labels (pink/red theme)
        if mobGuiRef then
            for _, btn in ipairs(mobGuiRef:GetDescendants()) do
                if btn:IsA("TextLabel") and btn.Text:upper():find("BAT") or btn.Text:upper():find("AIMBOT") or btn.Text:upper():find("ANTI") then
                    btn.TextColor3 = redColor
                end
            end
        end
    end
...
print("FURY DUELS LOADED - SLOW RED CYCLING ON BLACK BACKGROUND")
